...
// - Remitm logo (loaded from /assets/images/remitm-logo.svg)
...
// - Footer with payment note and remitm.com link
//
...
const LOGO_URL = 'assets/images/remitm-logo.svg';
...
const COLORS = {
  ...
  accent: [30, 64, 175] as [number, number, number],        // blue used for "BILLED TO" / "FROM" / remitm.com
  navy: [27, 58, 107] as [number, number, number]           // Remitm brand navy
};
...
export async function generateStripeStyleInvoice(
  ...
  // Native Remitm WebView bridge: post base64 so the app saves to Downloads; else browser download.
...
function drawTextWordmark(doc: any): void {
  ...
  doc.text('Remitm Money Transfer', LEFT_X, 62);
}
...
function drawParties(doc: any, profile: UserResponse | null): void {
  ...
  const fromLines: string[] = [
    'Remitm Money Transfer Ltd.',
    '193 Seven Sisters Road',
    'Finsbury Park, London, N4 3NG',
    'United Kingdom'
  ];
  const fromEmail = 'support@remitm.com';
...
function drawTotalsAndThanks(doc: any, startY: number, txn: TransactionResponse): void {
  ...
  doc.setFontSize(11);
  doc.setTextColor(...COLORS.ink);
  doc.text('Thank you for choosing Remitm Money Transfer.', LEFT_X, startY);
  doc.setTextColor(...COLORS.sub);
  doc.splitTextToSize(
...
function drawFooter(doc: any): void {
  ...
  doc.setFontSize(10);
  doc.setTextColor(...COLORS.accent);
  doc.text('remitm.com', RIGHT_X, footerY + 22, { align: 'right' });
}
