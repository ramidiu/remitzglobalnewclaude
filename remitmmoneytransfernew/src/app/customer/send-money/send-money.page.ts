...
export class SendMoneyPage implements OnInit, OnDestroy, ViewWillEnter {
  ...
  ionViewWillEnter(): void {
    ...
    if (beneficiaryId) {
      try {
        const draft = sessionStorage.getItem('remitm_send_draft');
        if (draft) {
  ...
  goToAddRecipient(): void {
    ...
    try {
      sessionStorage.setItem('remitm_send_draft', JSON.stringify({
        sendAmount: this.sendAmount,
        deliveryMethod: this.selectedDeliveryMethod,
      ...
      next: (freshQuote) => {
        ...
        this.transactionService.create({
          ...
          next: (txn: any) => {
            ...
            localStorage.setItem('remitm_last_txn', JSON.stringify(txnData));
            // Remember the chosen method so the return-callback page confirms with the right gateway.
            try { localStorage.setItem('remitm_pay_method', this.selectedPaymentMethod); } catch {}
            // Transaction created — drop the saved draft so its amount can't leak into a later send.
            try { sessionStorage.removeItem('remitm_send_draft'); } catch {}
  ...
  // confirms with /api/fire/update-payment (it reads remitm_pay_method).
  ...
  private payWithCard(txn: any): void {
    ...
    const fields: Array<[string, string]> = [
      ['sitereference',         'test_remitm107329'],
      ['stprofile',             'default'],
      ['currencyiso3a',         'GBP'],
