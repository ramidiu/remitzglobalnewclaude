...
export class TrustCallbackPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    try {
      const stored = localStorage.getItem('remitm_last_txn');
      if (stored) {
    ...
    this.route.queryParamMap.subscribe(params => {
      ...
      const payMethod = (localStorage.getItem('remitm_pay_method') || 'CARD');
  ...
  private confirmFireWithBackend(referenceNumber: string, paymentUuid: string): void {
    ...
    this.http.post<{ status: string }>(
      ...
      next: (res) => {
        ...
        if (this.isSuccess) {
          localStorage.removeItem('remitm_last_txn');
          localStorage.removeItem('remitm_pay_method');
        }
      },
      error: () => {
