#!/usr/bin/env python3
"""
Hash all plain text passwords in remitz.users with BCrypt.
...
print("Fetching plain text passwords from remitz.users...")

# Get all users with plain text passwords (not starting with $2)
result = subprocess.run(
    ["docker", "exec", "remitz-mysql", "mysql", "-uroot", "-proot", "remitz",
     "-N", "-e",
...
"""
    res = subprocess.run(
        ["docker", "exec", "remitz-mysql", "mysql", "-uroot", "-proot", "remitz", "-e", sql],
...
result2 = subprocess.run(
    ["docker", "exec", "remitz-mysql", "mysql", "-uroot", "-proot", "remitz", "-N", "-e",
