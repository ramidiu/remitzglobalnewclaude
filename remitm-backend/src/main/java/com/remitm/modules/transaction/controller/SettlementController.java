...
import com.remitm.modules.transaction.repository.SettlementRepository;
...
public class SettlementController {
    ...
    public ResponseEntity<ApiResponse<Settlement>> initiatePayinToAdmin(
        ...
        if (bodyPartnerId != null) {
            ...
            partner = payinPartnerRepository.findById(pid)
                    .orElseThrow(() -> new RemitmException("Pay-in partner not found: " + pid, HttpStatus.NOT_FOUND));
            ...
            partner = payinPartnerRepository.findByUserId(userId)
                    .orElseThrow(() -> new RemitmException("Pay-in partner not found", HttpStatus.NOT_FOUND));
        }
    ...
    public ResponseEntity<ApiResponse<Settlement>> approveSettlement(@PathVariable Long id,
        ...
        if (!"PENDING".equals(settlement.getStatus())) {
            throw new RemitmException("Settlement is not in PENDING status", HttpStatus.BAD_REQUEST);
        }
