...
public class BeneficiaryService {
    ...
    private void validateBeneficiaryFields(CreateBeneficiaryRequest r) {
        if (blank(r.getFullName()))
            throw new RemitmException("Beneficiary full name is required", HttpStatus.BAD_REQUEST);
        DeliveryMethod dm = r.getDeliveryMethod();
        if (dm == DeliveryMethod.BANK_DEPOSIT) {
            if (blank(r.getBankName()))
                throw new RemitmException("Bank name is required for bank transfers", HttpStatus.BAD_REQUEST);
            if (blank(r.getAccountNumber()) && blank(r.getIban()))
                throw new RemitmException("Account number (or IBAN) is required for bank transfers", HttpStatus.BAD_REQUEST);
        } else if (dm == DeliveryMethod.MOBILE_WALLET) {
            if (blank(r.getMobileNumber()))
                throw new RemitmException("Mobile number is required for mobile money", HttpStatus.BAD_REQUEST);
        } else if (dm == DeliveryMethod.CASH_PICKUP) {
            if (blank(r.getBankName()))
                throw new RemitmException("Collection point is required for cash pickup", HttpStatus.BAD_REQUEST);
        }
    }
    ...
    public BeneficiaryResponse getBeneficiary(Long userId, Long beneficiaryId) {
        return beneficiaryRepository.findByUserIdAndId(userId, beneficiaryId)
                .map(this::mapToResponse)
                .orElseThrow(() -> new com.remitm.common.exception.ResourceNotFoundException("Beneficiary", "id", beneficiaryId));
    }
