...
public class PayinPartnerController {
    ...
    public PayinPartnerController(PayinPartnerRepository payinPartnerRepository,
                                   ...
                                   com.remitm.modules.auth.repository.UserRepository userRepository,
    ...
    public ResponseEntity<ApiResponse<TransactionEntity>> markFundsReceived(
        ...
        if (!partner.getId().equals(tx.getPayinPartnerId())) {
            throw new RemitmException("Transaction not assigned to this pay-in partner", HttpStatus.FORBIDDEN);
        }
    ...
    private PayinPartner resolvePayinPartner(Long userId, Long adminPartnerId) {
        if (adminPartnerId != null) {
            return payinPartnerRepository.findById(adminPartnerId)
                    .orElseThrow(() -> new RemitmException("Pay-in partner not found: " + adminPartnerId, HttpStatus.NOT_FOUND));
        }
        if (userId != null) {
        ...
        try {
            ...
            if (auth != null && auth.getName() != null) {
                ...
                java.util.Optional<com.remitm.modules.auth.entity.UserEntity> userOpt =
                        userRepository.findByUuid(uuid);
                if (userOpt.isPresent()) {
                    com.remitm.modules.auth.entity.UserEntity u = userOpt.get();
        ...
        throw new RemitmException("Pay-in partner not resolvable for caller", HttpStatus.FORBIDDEN);
    }
}
