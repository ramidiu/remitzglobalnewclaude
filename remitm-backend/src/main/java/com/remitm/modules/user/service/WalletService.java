...
import com.remitm.modules.user.repository.WalletLedgerRepository;
import com.remitm.modules.user.repository.WalletRepository;
...
public class WalletService {
        ...
        return walletRepository.findByUserId(userId)
                .orElseGet(() -> {
                    userRepository.findById(userId)
                            .orElseThrow(() -> new RemitmException("User not found with id: " + userId, HttpStatus.NOT_FOUND));
    ...
    public WalletEntity getWalletByUserId(Long userId) {
        return walletRepository.findByUserId(userId)
                .orElseThrow(() -> new RemitmException("Wallet not found for userId: " + userId, HttpStatus.NOT_FOUND));
    }
    ...
    public WalletEntity debit(Long userId, BigDecimal amount, String description, String referenceId, String referenceType) {
        ...
        if (wallet.getBalance().compareTo(amount) < 0) {
            throw new RemitmException("Insufficient wallet balance. Available: " + wallet.getBalance() + ", Required: " + amount, HttpStatus.BAD_REQUEST);
        }
