...
--           Migrates data from remitm_old → remitm.
--
-- Call with dry_run = 1 to test (ROLLBACK at end, shows validation report).
-- Call with dry_run = 0 to commit for real.
--
-- Execute inside container:
--   docker exec -i remitm-mysql mysql -uroot -proot remitm < 02_migration_procedure.sql
-- =============================================================================
...
CREATE PROCEDURE run_migration(IN dry_run TINYINT(1))
    COMMENT 'Full data migration from remitm_old to remitm. dry_run=1 rolls back, dry_run=0 commits.'
BEGIN
    ...
    BEGIN
        ...
        INSERT INTO remitm.migration_log (phase, table_name, status, message)
    ...
    REPLACE INTO remitm.migration_summary (phase, attempted, succeeded, skipped, failed)
    ...
    -- when the source table is absent (the INSERT inside a non-taken IF is never resolved).
    IF (SELECT COUNT(*) FROM information_schema.tables
          WHERE table_schema = 'remitm_old' AND table_name = 'remit_one') > 0 THEN

        INSERT IGNORE INTO remitm.remit_one (
        ...
        SELECT
            ...
            MESSAGE
        FROM remitm_old.remit_one;

        SET v_inserted  = ROW_COUNT();
        SET v_old_count = (SELECT COUNT(*) FROM remitm_old.remit_one);
    ...
    ELSE
        ...
        VALUES (v_phase, 'remit_one', 'SKIPPED',
                'Source table remitm_old.remit_one not present — phase skipped');
        REPLACE INTO remitm.migration_summary (phase, attempted, succeeded, skipped, failed)
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM remitm.users nu WHERE nu.email = u.USER_EMAIL
    );

    SET v_inserted  = ROW_COUNT();
    SET v_old_count = (SELECT COUNT(*) FROM remitm_old.`user`
                       WHERE USER_EMAIL IS NOT NULL AND TRIM(USER_EMAIL) != '');

    -- Populate migration_user_map for all old users (including pre-existing emails)
    INSERT IGNORE INTO remitm.migration_user_map (old_user_id, new_user_id, old_email)
    SELECT u.USER_ID, nu.id, u.USER_EMAIL
    FROM remitm_old.`user` u
    JOIN remitm.users nu ON nu.email = u.USER_EMAIL
    ...
    VALUES (v_phase, 'users', 'OK',
            CONCAT('Inserted: ', v_inserted,
                   ...
                   ' | Mapped: ', (SELECT COUNT(*) FROM remitm.migration_user_map)));
    ...
    -- code so the profile shows a real country instead of blank. (delta: NULL-only)
    -- =========================================================================
    SET v_phase = 'PHASE_2B_USER_COUNTRY';

    UPDATE remitm.users nu
    JOIN remitm.migration_user_map m ON m.new_user_id = nu.id
    JOIN remitm_old.`user` u         ON u.USER_ID = m.old_user_id
    JOIN remitm_old.country c        ON c.COUNTRY_ID = u.COUNTRY_ID
    SET nu.country = TRIM(c.COUNTRY_ISO)
    WHERE nu.id > 6
      AND nu.country IS NULL
    ...
    INSERT IGNORE INTO remitm.wallets (
        user_id,
        balance,
        currency,
        is_active,
    ...
    SELECT
        ...
        0
    FROM remitm.migration_user_map m
    JOIN remitm_old.`user` u ON u.USER_ID = m.old_user_id
    WHERE NOT EXISTS (
        SELECT 1 FROM remitm.wallets w WHERE w.user_id = m.new_user_id
    );
    ...
    REPLACE INTO remitm.migration_summary (phase, attempted, succeeded, skipped, failed)
    VALUES (v_phase, (SELECT COUNT(*) FROM remitm.migration_user_map), v_inserted, 0, 0);
    ...
    JOIN (
        ...
        FROM remitm_old.user_addr ua
        INNER JOIN (
            SELECT USER_ID, MAX(USER_ADDR_ID) AS max_id
            FROM remitm_old.user_addr
            GROUP BY USER_ID
    ...
    SET
        ...
        nu.country_of_residence = (
            SELECT TRIM(cc.COUNTRY_ISO) FROM remitm_old.country cc
    ...
    INSERT INTO remitm.kyc_documents (
        user_id,
        document_type,
        file_path,
        status,
    ...
    JOIN remitm.migration_user_map m ON m.old_user_id = ua.USER_ID
    WHERE NOT EXISTS (
        SELECT 1 FROM remitm.kyc_documents kd
    ...
    SELECT
        ...
        NOW()
    FROM remitm_old.user_identity ui
    JOIN remitm.migration_user_map m ON m.old_user_id = ui.USER_ID
    WHERE NOT EXISTS (
        SELECT 1 FROM remitm.kyc_documents kd
        WHERE kd.user_id          = m.new_user_id
    ...
    SET v_inserted  = ROW_COUNT();
    SET v_old_count = (SELECT COUNT(*) FROM remitm_old.user_identity);
    ...
    WHERE ui.ID_PROOF_BACK IS NOT NULL
      ...
      -- DELTA GUARD: don't re-insert a back-of-ID doc that already exists for this user
      AND NOT EXISTS (
          SELECT 1 FROM remitm.kyc_documents kd
          WHERE kd.user_id = m.new_user_id
            AND kd.document_number = CONCAT(COALESCE(NULLIF(TRIM(ui.IDENTITY_DOC_NUM),''), 'UNKNOWN'), '_BACK')
      );
    ...
    INSERT INTO remitm.kyc_verifications (
        user_id,
        verification_type,
        provider,
        status,
    ...
    FROM (
        ...
        FROM remitm_old.user_identity
        GROUP BY USER_ID
    ) ui_agg
    JOIN remitm.migration_user_map m ON m.old_user_id = ui_agg.USER_ID
    WHERE NOT EXISTS (
        SELECT 1 FROM remitm.kyc_verifications kv
    ...
    -- (delta: NULL-only so re-runs don't overwrite edits)
    -- =========================================================================
    ...
    INSERT INTO remitm.beneficiaries (
        user_id,
        full_name,
        email,
        address,
    ...
    SELECT
        ...
        -- Map the old DEST_COUNTRY_CODE → ISO2 (messy: country NAME, sometimes numeric id, or NULL).
        -- There is no beneficiary.COUNTRY_ID column.
        COALESCE(
            (SELECT TRIM(c.COUNTRY_ISO) FROM remitm_old.country c
        ...
        NOW()
    FROM remitm_old.beneficiary b
    JOIN remitm.migration_user_map m ON m.old_user_id = b.SENDING_CUST_USER_ID
    -- DELTA GUARD: only insert beneficiaries not already migrated (tracked by old id)
    WHERE b.BENEFICIARY_ID NOT IN (
        SELECT old_beneficiary_id FROM remitm.migration_beneficiary_map
    );

    SET v_inserted  = ROW_COUNT();
    SET v_old_count = (SELECT COUNT(*) FROM remitm_old.beneficiary
                       WHERE SENDING_CUST_USER_ID IN
                            (SELECT old_user_id FROM remitm.migration_user_map));

    -- Build beneficiary mapping table
    -- Match: old BENEFICIARY_ID → new beneficiaries.id using user + name combination
    INSERT IGNORE INTO remitm.migration_beneficiary_map
        (old_beneficiary_id, new_beneficiary_id)
    SELECT
        b.BENEFICIARY_ID,
        (
            SELECT nb.id
            FROM remitm.beneficiaries nb
            JOIN remitm.migration_user_map mm2 ON mm2.new_user_id = nb.user_id
    ...
    FROM remitm_old.beneficiary b
    WHERE b.SENDING_CUST_USER_ID IN (SELECT old_user_id FROM remitm.migration_user_map)
      AND (
          SELECT nb.id FROM remitm.beneficiaries nb
          JOIN remitm.migration_user_map mm2 ON mm2.new_user_id = nb.user_id
          WHERE mm2.old_user_id = b.SENDING_CUST_USER_ID
            AND nb.full_name = TRIM(CONCAT(COALESCE(b.BENIF_FN,''), ' ', COALESCE(b.BENIF_LN,'')))
    ...
    VALUES (v_phase, 'beneficiaries', 'OK',
            CONCAT('Inserted: ', v_inserted,
                   ' | Mapped: ', (SELECT COUNT(*) FROM remitm.migration_beneficiary_map)));
    ...
    INSERT INTO remitm.transactions (
        reference_number,
        idempotency_key,
        sender_id,
        beneficiary_id,
    ...
    SELECT
        ...
        COALESCE(
            (SELECT cor.id FROM remitm.corridors cor
    ...
    FROM remitm_old.`transaction` t
    JOIN  remitm.migration_user_map m ON m.old_user_id = t.SENDING_CUST_USER_ID
    LEFT JOIN remitm_old.transaction_receiving_branch tr
           ON tr.TRANSACTION_ID = t.TRANSACTION_ID
    LEFT JOIN remitm.migration_beneficiary_map bm
           ON bm.old_beneficiary_id = tr.BENEFICIARY_ID
    WHERE NOT EXISTS (
        SELECT 1 FROM remitm.transactions nt
        WHERE nt.reference_number = t.TRANSACTION_ID
    );

    SET v_inserted  = ROW_COUNT();
    SET v_old_count = (SELECT COUNT(*) FROM remitm_old.`transaction` t
                       WHERE EXISTS (
                           SELECT 1 FROM remitm.migration_user_map m
                           WHERE m.old_user_id = t.SENDING_CUST_USER_ID
                       ));
    SET v_skipped   = (SELECT COUNT(*) FROM remitm_old.`transaction`) - v_old_count;
    ...
    REPLACE INTO remitm.migration_summary (phase, attempted, succeeded, skipped, failed)
    VALUES (v_phase, (SELECT COUNT(*) FROM remitm_old.`transaction`),
    ...
    INSERT INTO remitm.transaction_status_history (
        transaction_id,
        from_status,
        to_status,
        actor_type,
    ...
    SELECT
        ...
        nt.status,
        'SYSTEM',
        'Migrated initial status from remitm_old',
        nt.created_at
    FROM remitm.transactions nt
    WHERE nt.reference_number LIKE 'TXN%'
      AND NOT EXISTS (
          SELECT 1 FROM remitm.transaction_status_history tsh
    ...
    FROM (
        SELECT 1 AS `#`, 'remit_one records' AS metric,
            (SELECT COUNT(*) FROM remitm_old.remit_one) AS old_val,
            (SELECT COUNT(*) FROM remitm.remit_one) AS new_val
        UNION ALL
        SELECT 2, 'users migrated (excl. seed)',
            (SELECT COUNT(*) FROM remitm_old.`user` WHERE USER_EMAIL IS NOT NULL AND TRIM(USER_EMAIL)!=''),
            (SELECT COUNT(*) FROM remitm.users WHERE id > 6)
        UNION ALL
        SELECT 3, 'user map entries',
            (SELECT COUNT(*) FROM remitm_old.`user` WHERE USER_EMAIL IS NOT NULL AND TRIM(USER_EMAIL)!=''),
            (SELECT COUNT(*) FROM remitm.migration_user_map)
        UNION ALL
        SELECT 4, 'wallets created',
            (SELECT COUNT(*) FROM remitm.migration_user_map),
            (SELECT COUNT(*) FROM remitm.wallets WHERE user_id IN
                (SELECT new_user_id FROM remitm.migration_user_map))
        UNION ALL
        SELECT 5, 'users with address updated',
            (SELECT COUNT(DISTINCT USER_ID) FROM remitm_old.user_addr),
            (SELECT COUNT(*) FROM remitm.users WHERE id > 6 AND address_line_1 IS NOT NULL)
        UNION ALL
        SELECT 6, 'kyc_documents (identity front)',
            (SELECT COUNT(*) FROM remitm_old.user_identity),
            (SELECT COUNT(*) FROM remitm.kyc_documents WHERE user_id > 6 AND document_type != 'PROOF_OF_ADDRESS' AND document_number NOT LIKE '%_BACK')
        UNION ALL
        SELECT 7, 'kyc_verifications (identity)',
            (SELECT COUNT(DISTINCT USER_ID) FROM remitm_old.user_identity),
            (SELECT COUNT(*) FROM remitm.kyc_verifications WHERE user_id > 6)
        UNION ALL
        SELECT 8, 'users TIER_2 (fully verified)',
            (SELECT COUNT(DISTINCT USER_ID) FROM remitm_old.user_identity
             WHERE LOWER(ID_VERIFIED_STATUS) IN ('full','verified')),
            (SELECT COUNT(*) FROM remitm.users WHERE kyc_tier = 'TIER_2' AND id > 6)
        UNION ALL
        SELECT 9, 'beneficiaries migrated',
            (SELECT COUNT(*) FROM remitm_old.beneficiary WHERE SENDING_CUST_USER_ID
                IN (SELECT old_user_id FROM remitm.migration_user_map)),
            (SELECT COUNT(*) FROM remitm.beneficiaries)
        UNION ALL
        SELECT 10, 'transactions migrated',
            (SELECT COUNT(*) FROM remitm_old.`transaction` WHERE SENDING_CUST_USER_ID
                IN (SELECT old_user_id FROM remitm.migration_user_map)),
            (SELECT COUNT(*) FROM remitm.transactions WHERE reference_number LIKE 'TXN%')
        UNION ALL
        SELECT 11, 'PAID transactions (financial check)',
            (SELECT COUNT(*) FROM remitm_old.`transaction` WHERE LOWER(STATUS) = 'paid'),
            (SELECT COUNT(*) FROM remitm.transactions WHERE status = 'PAID'
                AND reference_number LIKE 'TXN%')
        UNION ALL
        SELECT 12, 'GBP send_amount sum (PAID)',
            ROUND(COALESCE((SELECT SUM(SENDING_AMOUNT) FROM remitm_old.`transaction`
                            WHERE LOWER(STATUS) = 'paid'), 0), 2),
            ROUND(COALESCE((SELECT SUM(send_amount) FROM remitm.transactions
    ...
    FROM remitm.transactions t
    LEFT JOIN remitm.users u ON t.sender_id = u.id
    WHERE u.id IS NULL
