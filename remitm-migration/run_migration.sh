...
#
# Usage:
#   bash run_migration.sh setup         # Step 0: import old SQL into remitm_old
...
mysql_remitm() {
    docker exec -i "$CONTAINER" mysql -u"$MYSQL_USER" -p"$MYSQL_PASS" remitm 2>/dev/null
}
...
case "$1" in
  ...
  live)
    ...
    mysql_remitm < "$SCRIPT_DIR/04_run_live.sql"
    echo ""
  ...
  validate)
    ...
    mysql_remitm < "$SCRIPT_DIR/05_validation.sql"
    ;;

  rollback)
    header "Step 6: ROLLBACK — removing migrated data"
    echo "WARNING: This will delete all migrated data from remitm."
    read -p "Type ROLLBACK to confirm: " confirm
    if [ "$confirm" != "ROLLBACK" ]; then
        echo "Aborted."
        exit 0
    fi
    mysql_remitm < "$SCRIPT_DIR/06_rollback.sql"
    ;;
  ...
  *)
    ...
    echo ""
    echo "Steps:"
    echo "  setup          Import old SQL into remitm_old staging DB"
    echo "  infrastructure Create support tables in remitm"
