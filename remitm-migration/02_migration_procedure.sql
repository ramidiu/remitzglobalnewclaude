    ...
    FROM (
        SELECT 1 AS `#`, 'remit_one records' AS metric,
            ...
            COALESCE((SELECT table_rows FROM information_schema.tables
                      WHERE table_schema='remitm_old' AND table_name='remit_one'), 0) AS old_val,
        ...
        SELECT 13, 'users with country set',
            (SELECT COUNT(*) FROM remitm_old.`user` u
             WHERE u.COUNTRY_ID IS NOT NULL
               AND EXISTS (SELECT 1 FROM remitm_old.country c WHERE c.COUNTRY_ID = u.COUNTRY_ID)
               AND u.USER_EMAIL IS NOT NULL AND TRIM(u.USER_EMAIL) != ''),
            (SELECT COUNT(*) FROM remitm.users WHERE id > 6 AND country IS NOT NULL)
        UNION ALL
        SELECT 14, 'users with date_of_birth set',
            (SELECT COUNT(DISTINCT USER_ID) FROM remitm_old.user_identity
             WHERE DOB REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}$'),
            (SELECT COUNT(*) FROM remitm.users WHERE id > 6 AND date_of_birth IS NOT NULL)
    ) chk;
