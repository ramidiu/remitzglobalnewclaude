...
--   docker exec -i remitm-mysql mysql -uroot -proot remitm < 05_validation.sql
-- =============================================================================
...
FROM (
    SELECT 'remit_one'          AS table_name,
        (SELECT COUNT(*) FROM remitm_old.remit_one) AS old_count,
        (SELECT COUNT(*) FROM remitm.remit_one)    AS new_count
    UNION ALL
    SELECT 'users (migrated)',
        (SELECT COUNT(*) FROM remitm_old.`user` WHERE USER_EMAIL IS NOT NULL),
    ...
    SELECT 'wallets',
        (SELECT COUNT(*) FROM remitm.migration_user_map),
        (SELECT COUNT(*) FROM remitm.wallets
         WHERE user_id IN (SELECT new_user_id FROM remitm.migration_user_map))
    UNION ALL
    SELECT 'kyc_documents (identity)',
        (SELECT COUNT(*) FROM remitm_old.user_identity),
        (SELECT COUNT(*) FROM remitm.kyc_documents
    ...
    SELECT 'beneficiaries',
        (SELECT COUNT(*) FROM remitm_old.beneficiary
         WHERE SENDING_CUST_USER_ID IN (SELECT old_user_id FROM remitm.migration_user_map)),
        (SELECT COUNT(*) FROM remitm.beneficiaries)
    UNION ALL
    SELECT 'transactions',
        (SELECT COUNT(*) FROM remitm_old.`transaction`
         WHERE SENDING_CUST_USER_ID IN (SELECT old_user_id FROM remitm.migration_user_map)),
...
SELECT
    ...
    COUNT(*) AS count
FROM remitm.kyc_documents kd
LEFT JOIN remitm.users u ON kd.user_id = u.id
...
SELECT
    ...
    m.old_user_id
FROM remitm.users u
JOIN remitm.migration_user_map m ON m.new_user_id = u.id
ORDER BY u.id
LIMIT 5;
